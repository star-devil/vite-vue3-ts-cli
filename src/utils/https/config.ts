...
import type { PureHttpResponse, IResponse } from './types';
 ...
 * @param response 响应数据
 */
const handleGeneralError = async (response: PureHttpResponse) => {
 ...
 * @param data 响应数据
 */
async function handleError(response: PureHttpResponse, data: IResponse<any>) {
...
export function handleResponseData(response: PureHttpResponse) {
  const { data } = response;
  try {
